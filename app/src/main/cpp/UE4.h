...
inline Matrix GetMatrix() {
    ...
    return *(Matrix*)((uintptr_t)Canvas + 0x270);
}

inline bool WorldToScreenBone(FVector Position, FVector2D *Screen, float screenWidth, float screenHeight) {
    ...
    ScreenW = 1 / ScreenW;
    float glWidth = screenWidth;
    float glHeight = screenHeight;
...
inline FVector MatrixToVector(Matrix matrix) {
    return {matrix.m[3][0], matrix.m[3][1], matrix.m[3][2]};
}

inline Matrix MatrixMultiplication(Matrix m1, Matrix m2) {
...
inline Matrix TransformToMatrix(FTransform transform) {
    Matrix matrix;
...
inline FVector TransformToLocation(FTransform c2w, FTransform transform) {
    return MatrixToVector(MatrixMultiplication(TransformToMatrix(transform), TransformToMatrix(c2w)));
}


inline const char *GetLootName(APickUpListWrapperActor *Loot) {
     switch (Loot->BoxType) {
...
*/
inline int skeletonByDistance2(int distance){
...
inline FVector WorldToRadar(float Yaw, FVector Origin, FVector LocalOrigin, float PosX, float PosY, Vector3 Size, bool & outbuff) {
    bool flag = false;
...
inline void VectorAnglesRadar(Vector3 & forward, FVector & angles) {
...
inline void RotateArrows(std::array<Vector3, 7>& points, float rotation) {
const auto points_center = (points.at(0) + points.at(1) + points.at(2)) / 3;
for (auto& point : points) {
point = point - points_center;
const auto temp_x = point.X;
...
inline FVector2D pushToScreenBorder(FVector2D Pos, FVector2D screen, int borders, int offset) {
    int x = (int)Pos.X;
    int y = (int)Pos.Y;
    if ((borders & 1) == 1) {
        y = 0 - offset;
...
inline int isOutsideSafezone(Vector2 pos, Vector2 screen) {
...
inline Vector2 pushToScreenBorder(Vector2 Pos, Vector2 screen, int borders, int offset) {
    int x = (int)Pos.X;
    int y = (int)Pos.Y;
    if ((borders & 1) == 1) {
        y = 0 - offset;
...
inline void AimAngle(FRotator &angles) {
    if (angles.Pitch > 180)
        angles.Pitch -= 360;
    if (angles.Pitch < -180)
        angles.Pitch += 360;
